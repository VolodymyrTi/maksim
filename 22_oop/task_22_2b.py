...
class CiscoTelnet:
    ...
    def send_config_commands(self, commands):
        ...
        self.telnet.read_until(b"(config)#", timeout=5).decode("utf-8")
        output = ""
        if type(commands) == str:
            self._write_line(commands)
            time.sleep(2)
            output = self.telnet.read_until(b"(config)#", timeout=5).decode("utf-8")
        else:
            for i in commands:
                self._write_line(i)
                out = self.telnet.read_until(b"(config)#", timeout=5).decode("utf-8")
                output = output + out
        return output
